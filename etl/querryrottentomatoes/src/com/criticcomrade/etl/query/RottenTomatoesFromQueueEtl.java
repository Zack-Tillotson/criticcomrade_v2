...
import com.criticcomrade.etl.query.db.RtActivityDao;
import com.criticcomrade.etl.query.db.RtQueueDao;

public class RottenTomatoesFromQueueEtl extends EtlThread {

    private final boolean noUpdates;

    public RottenTomatoesFromQueueEtl(Connection conn, int maxRuntime, boolean noUpdates)
    ...
    protected boolean haveReasonToQuit(List<String> reasons) {
        ...
        boolean tooManyApiCalls = (new RtActivityDao(conn).getNumberOfApiCallsSince(new Date(nowWhen.getTime()
                - API_THROTTLE_PERIOD)) >= API_THROTTLE_AMOUNT);
        if (tooManyApiCalls) {
     ...
     * time of this call). The order movies will be returned are: active movies (movies with a found date within the
     * active window but a last query date outside the stale window), then the newest movie on the queue with a null
     * last queried date, then the movie on the queue with the oldest last querried date
     * 
    ...
    protected String getNextIdToEtl() {
        ...
        if (!noUpdates) {
            ...
            List<String> activeMovies = rtQueueDao.getMovieActiveWithinTimePeriod(new Date((new Date()).getTime()
                    - ACTIVE_TIME_PERIOD_START), new Date((new Date()).getTime() - ACTIVE_TIME_PERIOD_END));
    ...
    public DataItem doEtlImpl(String id) {
        ...
        if (!RtQueueDao.getMovieLock(id, rtQueueDao)) {
            ...
            try {
                ...
                if (e.toString().startsWith("java.io.IOException: Server returned HTTP response code: 500 for URL: ")) { // Bad
                    // Id
